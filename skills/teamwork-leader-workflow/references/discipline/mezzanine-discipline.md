...
2. Storybook（`mezzanine.rytass.com`）— 若 sandbox 擋住 WebFetch，請使用者貼連結內容
...
User-level rules at `~/.claude/rules/mezzanine-ui.md` (if present) take precedence per user-instruction priority. This file is plugin's portable default for users without that file.
